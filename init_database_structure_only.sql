-- =====================================================
-- 智能代码知识沉淀平台 - 数据库表结构SQL脚本（仅表结构）
-- =====================================================
-- 说明：此脚本仅创建表结构，不包含默认数据
-- 适用于：已有数据库，只需要创建表结构的情况
-- 使用方法：
...
-- 审查统计视图
DROP VIEW IF EXISTS `v_review_statistics`;
CREATE VIEW `v_review_statistics` AS
SELECT 
...
-- 知识库统计视图
DROP VIEW IF EXISTS `v_knowledge_statistics`;
CREATE VIEW `v_knowledge_statistics` AS
SELECT 
